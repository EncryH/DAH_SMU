...
### MAVLink (UAV ↔ Companion Computer)

실제 [pymavlink](https://github.com/ArduPilot/pymavlink) 라이브러리를 사용한 **MAVLink 2.0** 프로토콜.

UAV(`uav/sitl_runner.py`)는 실제 **ArduPilot SITL(Software In The Loop)** 바이너리를 실행한다.  
ArduPilot은 실제 항공기 펌웨어로, 비행 물리 시뮬레이션·Fail-safe 로직·웨이포인트 자율비행을 처리한다.

```
ArduPlane SITL (TCP:5760)
...
- SITL → Companion: `HEARTBEAT`, `SYS_STATUS`, `GLOBAL_POSITION_INT`, `MISSION_ITEM_REACHED`
- GCS → UAV (명령): `COMMAND_LONG` (`MAV_CMD_NAV_LAND`, `MAV_CMD_NAV_RETURN_TO_LAUNCH` 등)
- GCS → UAV (연결 유지): `HEARTBEAT` 1Hz — 5초 이상 없으면 UAV Fail-safe LOITER 전환
